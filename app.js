(function () {
  ...
  function removeRequest(reference) {
    const requests = getRequests().filter(request => request.reference !== reference);
    localStorage.setItem('sevenscale-booking-requests', JSON.stringify(requests));
    updateRequestCount();
    renderRequests();
  ...
  function renderRequests() {
    ...
    list.innerHTML = requests.length ? requests.map(request => `
      <article class="request-item ${request.status === 'cancelled' ? 'is-cancelled' : ''}">
        ...
        <div class="request-actions">
          <span class="request-status ${request.status === 'cancelled' ? 'cancelled' : ''}">
            ${request.status === 'cancelled' ? 'Cancelled' : 'For confirmation'}
          </span>
          ${request.status === 'cancelled' ? '' : `<button class="cancel-request" type="button" data-cancel-request="${request.reference}">Cancel request</button>`}
        </div>
    ...
    list.querySelectorAll('[data-cancel-request]').forEach(button => {
      button.addEventListener('click', () => {
        const reference = button.dataset.cancelRequest;
        const request = getRequests().find(item => item.reference === reference);
        if (!request) return;
        if (window.confirm(`Cancel your request for ${request.resort}?`)) removeRequest(reference);
      });
    });
  }
